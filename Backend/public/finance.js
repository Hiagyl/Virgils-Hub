...
addRecordForm.addEventListener("submit", async (e) => {
  ...
  const txn = {
    ...
    type: recType.value.toLowerCase(), // 🔥 FIX: normalize type
    description: recDesc.value,
    amount: Number(recAmount.value),
    date: recDate.value,
  };

  if (!txn.type || !txn.amount || !txn.date) {
    alert("Please complete all required fields");
    return;
  }

  const result = await saveTransaction(txn);
  if (!result) return; // 🔥 FIX: stop if save failed
...
function editRecord(id, type) {
  ...
  recDate.value = r.date.split("T")[0];
